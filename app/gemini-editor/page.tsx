...
export default function GeminiEditor() {
  ...
  const processImage = async () => {
    if (!currentFile || !editInstructions.trim()) return;
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          // Process the image with integrated billing
          const requestData = {
          ...
          const response = await fetch('/api/gemini-editor/edit', {
            method: 'POST',
            headers: {
          ...
          if (data.success && data.url) {
            ...
            if (data.balance !== undefined) {
              setError(null);
              // You could add a success message or update balance display here
            }
          } else {
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Укажите ваш API токен для доступа к Gemini Flash
                </p>
              </div>
              <Button
